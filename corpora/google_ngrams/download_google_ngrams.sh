...
do
    ...
    awk -F $'\t' 'BEGIN { OFS = FS } { a[tolower($1)]+=$3; } END {for (ngram in a) print ngram, a[ngram]}' googlebooks-eng-all-1gram-20120701-${letter} | sort > googlebooks-eng-all-1gram-20120701-${letter}_aggregated;
    ) &
done
wait
...
do
    ...
    do
        ...
        do
        (
        echo "aggregating googlebooks-eng-all-${n}gram-20120701-${first}${second}"
        awk -F $'\t' 'BEGIN { OFS = FS } { a[tolower($1)]+=$3; } END {for (ngram in a) print ngram, a[ngram]}' googlebooks-eng-all-${n}gram-20120701-${first}${second} | sort > googlebooks-eng-all-${n}gram-20120701-${first}${second}_aggregated;
        ) &
        done
        wait
    done
